...
export default function ChatWithCaptain({ onNavigate, captainName = 'Ahmed Hassan' }: ChatWithCaptainProps) {
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#E8F4F8' }}>
      ...
      <div className="px-6 pt-12 pb-4 flex-shrink-0"
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-white text-xl">{captainName}</h2>
            <p className="text-white/70 text-sm">Ocean Star • MVR 2,500</p>
          </div>
        </div>
      </div>
        ...
        {messages.map((msg) => (
          ...
          >
            <div
              ...
              style={{
                backgroundColor: msg.sender === 'customer' ? '#0A2463' : '#FFFFFF',
                color: msg.sender === 'customer' ? '#FFFFFF' : '#0A2463'
              }}
            >
              <p className="mb-1">{msg.text}</p>
              <p className={`text-xs ${msg.sender === 'customer' ? 'text-white/60' : 'text-gray-400'}`}>
                {msg.timestamp}
              </p>
            </div>
          </div>
